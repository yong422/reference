...
CSmtp::~CSmtp()
{
  ...
  std::vector<Recipient> nullVec;
  std::vector<std::string> nullVec2;
  Recipients.erase(Recipients.begin(), Recipients.end());
  Recipients.swap(nullVec);
  CCRecipients.erase(CCRecipients.begin(), CCRecipients.end());
  CCRecipients.swap(nullVec);
  BCCRecipients.erase(BCCRecipients.begin(), BCCRecipients.end());
  BCCRecipients.swap(nullVec);
  Attachments.erase(Attachments.begin(), Attachments.end());
  Attachments.swap(nullVec2);
  MsgBody.erase(MsgBody.begin(), MsgBody.end());
  MsgBody.swap(nullVec2);
#ifndef LINUX
  WSACleanup();
#endif
}
...
//  MsgBody.insert(MsgBody.end(),Text);
  //! Message를 한글로 전송할수있도록 인코딩추가.
  MsgBody.insert(MsgBody.end(),base64_encode(reinterpret_cast<const unsigned char*>(Text), strlen(Text)));
}
...
void CSmtp::ModMsgLine(unsigned int Line,const char* Text)
{
  if(Text)
  {
    ...
    //! Message를 한글로 전송할수 있도록 인코딩추가
    MsgBody.at(Line) = base64_encode(reinterpret_cast<const unsigned char*>(Text), strlen(Text));
  }
}
...
//              JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
//! 헤더의 제목, From name의 한글전송을위해 인코딩추가
void CSmtp::FormatHeader(char* header)
{
  ...
  if(!m_sMailFrom.size())
    throw ECSmtp(ECSmtp::UNDEF_MAIL_FROM);
  strcat(header,"From: \"=?utf-8?B?");
  if(m_sNameFrom.size())
    strcat(header, 
      base64_encode(reinterpret_cast<const unsigned char*>(m_sNameFrom.c_str()), m_sNameFrom.size()).c_str());
  strcat(header,"?=\" <");
  if(m_sNameFrom.size())
  ...
  if(!m_sSubject.size()) {
    ...
    strcat(header, 
      base64_encode(reinterpret_cast<const unsigned char*>(m_sSubject.c_str()), m_sSubject.size()).c_str());
    strcat(header, "?=\r\n");
  }
...
const char* CSmtp::GetLocalHostName()
{
  ...
  if((pstr = new char[255]) == NULL)
    throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
  if(gethostname(pstr,255) == SOCKET_ERROR)
  {
    delete[] pstr;
    throw ECSmtp(ECSmtp::WSA_HOSTNAME);
  }
  //fprintf(stdout, "host : %s\n", str);
  m_sLocalHostName.assign(pstr);
  delete[] pstr;
...
void CSmtp::SetTextType(const char* text)
{
  m_sTextType.erase();
  m_sTextType.insert(0,text);
}
